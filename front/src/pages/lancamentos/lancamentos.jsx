...
function ContaModal({ isOpen, onClose, onSave, initialData, empresas, entidades, categorias, centrosCusto, contasBancarias, bancoMap }) {
...
function Lancamentos() {
  ...
  const [baixaErrors, setBaixaErrors] = useState({});

  const carregarDados = useCallback(async () => {
    try {
      setLoading(true); setError('');
      const [ts, emps, ents, cats, centros, contas, bnks] = await Promise.all([
        listarTitulos(), listarEmpresas(), listarEntidades(),
        listarCategoriasFinanceiras(), listarCentrosCusto(),
        listarContasBancarias(), listarBancos(),
      ]);
      setTitulos(ts); setEmpresas(emps); setEntidades(ents);
      setCategorias(cats); setCentrosCusto(centros);
      setContasBancarias(contas); setBancos(bnks);
    } catch (e) {
  ...
  const contaNome = (id) => {
    const c = contasBancarias.find((x) => String(x.id) === String(id));
    return c ? `${bancoMap.get(String(c.bancoId)) || 'Banco'} - ${c.agencia}/${c.conta}` : '-';
  };
  ...
  function handleBaixaClick(item) {
    ...
    setBaixaContaId(String(item.contaBancariaId || ''));
    setBaixaValor(String(item.valorSaldo || ''));
    setBaixaErrors({});
    onBaixaOpen();
  }
  ...
  async function handleConfirmBaixa() {
    ...
    const errs = {};
    if (!baixaContaId) errs.conta = 'Conta bancária é obrigatória.';
    const valorNum = Number(String(baixaValor).replace(',', '.'));
    if (!baixaValor || Number.isNaN(valorNum) || valorNum <= 0) {
      errs.valor = 'Informe um valor válido maior que zero.';
    } else if (valorNum > Number(baixaTarget.valorSaldo)) {
      errs.valor = 'Valor não pode ser maior que o saldo restante.';
    }
    if (!dataBaixa) errs.data = 'A data da baixa é obrigatória.';
    if (Object.keys(errs).length) { setBaixaErrors(errs); return; }
    try {
      await registrarBaixa(baixaTarget.id, {
        conta_bancaria_id: Number(baixaContaId),
        valor: valorNum,
  ...
  const contasBaixaFiltradas = contasBancarias.filter((c) => !baixaTarget?.empresaId || String(c.empresaId) === String(baixaTarget.empresaId));
  ...
  return (
    <Box p="8" w="100%">
      <SimpleGrid columns={{ base: 1, md: 2, xl: 4 }} spacing="6" mb="8">
        ...
        <StatCard title="Previsão de Caixa" value={formatCurrency(saldoAtual + totalReceber - totalPagar)} isPositive={saldoAtual + totalReceber - totalPagar >= 0} />
        <StatCard title="Total a Receber" value={formatCurrency(totalReceber)} isPositive />
      ...
      <Box bg="white" p="6" borderRadius="xl" boxShadow="sm">
        <Flex justifyContent="space-between" alignItems="center" mb="6">
          <Flex gap="3">
            <Button variant={tab === 'PAGAR' ? 'solid' : 'outline'} onClick={() => setTab('PAGAR')}>Contas a Pagar</Button>
            <Button variant={tab === 'RECEBER' ? 'solid' : 'outline'} onClick={() => setTab('RECEBER')}>Contas a Receber</Button>
          </Flex>
          <Button bg="#61B4DD" color="white" leftIcon={<LuPlus />} onClick={handleOpenAdd}>Novo Título</Button>
        </Flex>

        {loading && <Flex justify="center" py="10"><Spinner /></Flex>}
        {error && <Flex direction="column" align="center" py="10" gap="3"><Text color="red.500">{error}</Text><Button onClick={carregarDados}>Tentar novamente</Button></Flex>}
        {!loading && !error && (
          <TableContainer>
            <Table size="sm">
              <Thead>
                <Tr><Th>Descrição</Th><Th>Empresa</Th><Th>Entidade</Th><Th>Categoria</Th><Th>Conta</Th><Th>Vencimento</Th><Th>Valor Original</Th><Th>Saldo</Th><Th>Status</Th><Th>Ações</Th></Tr>
              </Thead>
              <Tbody>
                {list.length === 0
                  ? <Tr><Td colSpan={10} textAlign="center" py="8">Nenhum título encontrado.</Td></Tr>
                  : list.map((t) => (
                    <Tr key={t.id}>
                      <Td>{t.descricao}</Td>
                      <Td>{empresaNome(t.empresaId)}</Td>
                      <Td>{entidadeNome(t.entidadeId)}</Td>
                      <Td>{categoriaNome(t.categoriaId)}</Td>
                      <Td>{contaNome(t.contaBancariaId)}</Td>
                      <Td>{formatDate(t.dataVencimento)}</Td>
                      <Td>{formatCurrency(t.valorOriginal)}</Td>
                      <Td>{formatCurrency(t.valorSaldo)}</Td>
                      <Td><Badge bg={STATUS_META[t.status]?.bg || 'gray.100'} color={STATUS_META[t.status]?.color || 'gray.700'}>{STATUS_META[t.status]?.label || t.status}</Badge></Td>
                      <Td>
                        <Flex gap="1">
                          <IconButton isDisabled={t.status === 'CANCELADO'} icon={<LuPencil size={14} />} size="xs" variant="ghost" aria-label="Editar" onClick={() => handleEdit(t)} />
                          <Button size="xs" colorScheme="blue" variant="outline" isDisabled={['CANCELADO', 'PAGO', 'RECEBIDO'].includes(t.status)} onClick={() => handleBaixaClick(t)}>Baixa</Button>
                          <IconButton isDisabled={t.status === 'CANCELADO'} icon={<LuTrash2 size={14} />} size="xs" variant="ghost" colorScheme="red" aria-label="Cancelar" onClick={() => handleCancelClick(t)} />
                        </Flex>
                      </Td>
                    </Tr>
                  ))}
      ...
      <ContaModal isOpen={isOpen} onClose={onClose} onSave={handleSave} initialData={editItem} empresas={empresas} entidades={entidades} categorias={categorias} centrosCusto={centrosCusto} contasBancarias={contasBancarias} bancoMap={bancoMap} />

      {/* Modal de Baixa */}
      <Modal isOpen={isBaixaOpen} onClose={onBaixaClose} isCentered>
        <ModalOverlay backdropFilter="blur(4px)" />
        <ModalContent borderRadius="xl">
          <ModalHeader>Registrar baixa financeira</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb="6">
            <Flex direction="column" gap="4">
              <Text fontSize="sm" color="gray.600">
                Informe os dados do {baixaTarget?.tipo === 'PAGAR' ? 'pagamento' : 'recebimento'} do título.
              </Text>

              <FormControl>
                <FormLabel fontSize="sm">Saldo restante</FormLabel>
                <Input value={formatCurrency(baixaTarget?.valorSaldo)} isReadOnly />
              </FormControl>

              <FormControl isRequired isInvalid={!!baixaErrors.conta}>
                <FormLabel fontSize="sm">Conta Bancária</FormLabel>
                <Select
                  value={baixaContaId}
                  placeholder="Selecione a conta..."
                  onChange={(e) => { setBaixaContaId(e.target.value); setBaixaErrors((p) => ({ ...p, conta: '' })); }}
                >
                  {contasBaixaFiltradas.map((c) => (
                    <option key={c.id} value={c.id}>
                      {`${bancoMap.get(String(c.bancoId)) || 'Banco'} - ${c.agencia}/${c.conta}`}
                    </option>
                  ))}
                </Select>
                <FormErrorMessage>{baixaErrors.conta}</FormErrorMessage>
              </FormControl>

              <FormControl isRequired isInvalid={!!baixaErrors.valor}>
                <FormLabel fontSize="sm">Valor da baixa (R$)</FormLabel>
                <Input
                  type="number"
                  min="0.01"
                  step="0.01"
                  value={baixaValor}
                  onChange={(e) => { setBaixaValor(e.target.value); setBaixaErrors((p) => ({ ...p, valor: '' })); }}
                />
                <FormErrorMessage>{baixaErrors.valor}</FormErrorMessage>
              </FormControl>

              <FormControl>
                <FormLabel fontSize="sm">Data de vencimento</FormLabel>
                <Input value={formatDate(baixaTarget?.dataVencimento)} isReadOnly />
              </FormControl>

              <FormControl isRequired isInvalid={!!baixaErrors.data}>
                <FormLabel fontSize="sm">{baixaLabel}</FormLabel>
                <Input
                  type="date"
                  value={dataBaixa}
                  onChange={(e) => { setDataBaixa(e.target.value); setBaixaErrors((p) => ({ ...p, data: '' })); }}
                />
                <FormErrorMessage>{baixaErrors.data}</FormErrorMessage>
              </FormControl>

              <Flex justifyContent="flex-end" gap="3">
                <Button variant="ghost" onClick={onBaixaClose}>Cancelar</Button>
                <Button colorScheme="blue" onClick={handleConfirmBaixa}>Confirmar baixa</Button>
              </Flex>
            </Flex>
          </ModalBody>
        </ModalContent>
      ...
      <AlertDialog isOpen={isConfirmOpen} leastDestructiveRef={cancelRef} onClose={onConfirmClose}>
        <AlertDialogOverlay>
          <AlertDialogContent>
            <AlertDialogHeader>Cancelar título</AlertDialogHeader>
            <AlertDialogBody>Deseja cancelar "{cancelTarget?.descricao}"?</AlertDialogBody>
            <AlertDialogFooter>
              <Button ref={cancelRef} onClick={onConfirmClose}>Voltar</Button>
              <Button colorScheme="red" ml={3} onClick={handleConfirmCancel}>Cancelar título</Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialogOverlay>
      </AlertDialog>
